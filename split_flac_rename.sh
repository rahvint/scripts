...
# Introduction
# awk.9.30.16
# Fixed some pathing bugs and also added the feature of creating a folder of Album name
# and moving all split .flac to that folder.
#
echo
echo
echo                   "FLACCUE2FLAC"
...
echo
FILENAME=$(ls *.flac | head -1)
echo "$FILENAME"
FOLDERNAME=$(exiftool -s -S -Album "$FILENAME")
echo "$FOLDERNAME"
sudo mkdir "$FOLDERNAME"
sudo find . -type f -size -100M -name "*.flac" -exec mv {} "$FOLDERNAME" \;
echo
echo "Process ended."
done
